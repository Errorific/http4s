...
private[parser] trait HttpHeaderParser extends SimpleHeaders
                    with AcceptHeader
  ...
  // the java hashmap is measurably faster than the scala collections
  // Scala 2.11 has an AnyRefMap that is faster.
  private type Storage = util.HashMap[CaseInsensitiveString, HeaderParser]

  private var allParsers = new Storage()
  ...
  /** Add a parser to the global header parser registry
    *
    * Note that modification of the parser collection is not thread safe and care
    * should be take to ensure all parsers are registered at startup in a single threaded
    * manner.
    * 
  ...
  def addParser(key: CaseInsensitiveString, parser: HeaderParser): Option[HeaderParser] = {
    // elaborate clone etc instead of a `put` to avoid potential `ConcurrentModificationException`s.
    val next = allParsers.clone().asInstanceOf[Storage]
    val r = Option(next.put(key, parser))
    allParsers = next
    r
  }


  /** Remove the parser for the secified header key
    *
    * Note that modification of the parser collection is not thread safe and care
    * should be take to ensure all parsers are registered at startup in a single threaded
    * manner.
